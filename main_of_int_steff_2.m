%% 改进算法2：不使用导数信息且最高使用一阶C0信息的积分 - setff加速算法族 的改进方法
...
start_point = 0.06;
%% 求解过程
[root, step, choose, wList] = IntSteff2(max_step, max_f_step, max_x_step, start_point);
%% 结果展示
...
%% 改进Steffensen算法迭代流程
function [root, i, choose, wList] = IntSteff2(max_step, max_f_step, max_x_step, start_point)
% 计数
i = 1;
% 取出区间端点
[a, b] = Area();
...
while i <= max_step
    ...
    % 根据容忍限的表现确定alpha的迭代值
    if abs(z) > epsilon
        alpha = - ((f(w + z) - z)*(d - w)) / (z * (f(d) - z));
    else
        alpha = -1;
    end
    wnew = w + (d - w) * z / (z + alpha * f(d));
    if abs(wnew) == Inf
...
disp('您提交的 使用积分-Steff加速算法族优化算法计算单元方程的根 任务已经接近完成...');
disp('算法执行完毕, 打印结果...');
if choose == 0
...
num_plot_y = floor(length(plot_y)/2)+1;
% % % % % 
figure
subplot(2, 2, 1)
...
text(1.5, -20, disp_k, 'Color', [0, 0.5, 0.5], 'Fontname', 'Times New Roman','FontSize', 18);
set(gca,'FontSize', 15, 'Fontname', 'Times New Roman');
xlabel('Iteration Step', 'Fontname', 'Times New Roman','FontSize', 15);
...
text(2, 3, disp_k_high, 'Color', [0, 0.5, 0.5], 'Fontname', 'Times New Roman','FontSize', 18);
set(gca,'FontSize', 15, 'Fontname', 'Times New Roman');
xlabel('Iteration Step', 'Fontname', 'Times New Roman','FontSize', 15);
